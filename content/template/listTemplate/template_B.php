                        ...
                        <tr>
                                    ...
                                    <select name="eduadmin-subject">
										...
										<?php
											foreach ( $distinctSubjects as $subj => $val ) {
												echo '<option value="' . $subj . '"' . ( isset( $_REQUEST['eduadmin-subject'] ) && intval( $_REQUEST['eduadmin-subject'] ) == $subj ? " selected=\"selected\"" : "" ) . '>' . $val . '</option>';
											}
										?>
                                    </select>
                                </td>
                            ...
                            <td>
                                <input class="edu-searchTextBox" type="search" name="searchCourses" results="10"
                                       autosave="edu-course-search_<?php echo $apiUserId; ?>"
				...
				<?php
					...
					if ( ! empty( $attributes['subject'] ) ) {
						foreach ( $eds as $subject ) {
							if ( $subject->SubjectName == sanitize_text_field( $attributes['subject'] ) ) {
